# File from fairseq
# https://github.com/facebookresearch/fairseq/blob/0338cdc3094ca7d29ff4d36d64791f7b4e4b5e6e/examples/multilingual/data_scripts/binarize.py#L68
...
def encode_spm(model, direction, prefix='', splits=['train', 'test', 'valid'], pairs_per_shard=None):
    ...
    for split in splits:
        src_raw = f'{RAW_DIR}/{split}{prefix}.{direction}.{src}'
        tgt_raw = f'{RAW_DIR}/{split}{prefix}.{direction}.{tgt}'
        if os.path.exists(src_raw) and os.path.exists(tgt_raw):
            sp = spm.SentencePieceProcessor(model_file=model)
            output = sp.encode(load_text_file(src_raw), out_type=str)
            output = (' '.join(x) for x in output)
            write_text_file(output, f"{BPE_DIR}/{direction}{prefix}/{split}.bpe.{src}")

            output = sp.encode(load_text_file(tgt_raw), out_type=str)
            output = (' '.join(x) for x in output)
            write_text_file(output, f"{BPE_DIR}/{direction}{prefix}/{split}.bpe.{tgt}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--spm_vocab", nargs="*")
    ...
    SPM_VOCAB = tuple(args.spm_vocab)
